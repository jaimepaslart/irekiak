  ...
  try {
    ...
    if (existing) {
      await db.update(galleryContactOverrides)
        .set({ logoFilename: filename, updatedAt: now, updatedBy: 'admin' })
    ...
    await deleteGalleryLogo(filename)
    console.error('[gallery.logo.upload] DB write failed', { id, filename, err })
    throw createError({ statusCode: 500, statusMessage: 'Could not save logo override' })
  }
})
